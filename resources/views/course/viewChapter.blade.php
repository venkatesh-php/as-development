...
@section('content')
   ...
   <div class="container" id="chapter_data">
       <h2 class="text-center">{{$chapter->name}}</h2>
        <a  class="btn btn-primary pull-right" href="{{ url('course',['course_id'=>he($chapter->course_id)]) }}">Back</a>      
                        <hr>
        ...
        <?php $tcount = 0 ?>
            ...
            <div>
                ...
                @if ($task->uploads)
                     <a class="btn btn-default btn-xs" href="{{ url('download/'. $task->uploads)}}"  download="{{ $task->uploads}}"c/a>
                @else
